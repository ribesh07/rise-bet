...
//     if (!passwordsMatch || !currentPass) return;

//     try {
//       setErrorMsg("");

//       // Safely read token from localStorage (only in browser)
//       const token =
...
//         "https://api.playrise.vip/api/v1/users/update-password",
//         {
//           method: "POST",
//           headers,
//           body: JSON.stringify({
...
//       if (!data.success) {
//         setErrorMsg(data.message || "Current password incorrect.");
//         return;
//       }
...
//       {/* --- your session code remains exactly same --- */}
//     </motion.div>
//   );
// };
...
//   // 🔥 FIXED WORKING API — PUT /update-password
//   const handleUpdate = async () => {
//     if (!passwordsMatch || !currentPass) return;
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://api.playrise.vip/api/v1/users/update-password",
        {
          method: "POST", // backend accepts POST
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
          },
          body: JSON.stringify({
            oldPassword: currentPass,  // 🔥 FIXED — backend expects oldPassword
            newPassword: newPass,
          }),
        }
      );
