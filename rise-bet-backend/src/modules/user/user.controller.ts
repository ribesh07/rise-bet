import { Controller, Put, Body, UseGuards, Request, Post , Get, Param, Req, UploadedFile, UseInterceptors, BadRequestException, UploadedFiles, Query, UnauthorizedException} from '@nestjs/common';
...
async updatePassword(
  ...
  async uploadImage(@UploadedFile() file, @Req() req ,@Param('id') id: string) {
    ...
    if (
      !authUserId ||
      (authUserId !== idAsNum && req.user?.role !== 'ADMIN' && req.user?.role !== 'SUPERADMIN' && req.user?.isAdmin !== true)
    ) {
      throw new UnauthorizedException('You can only update your own image');
    }

    const userId = idAsNum;
