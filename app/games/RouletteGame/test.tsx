// components/ModernRoulette.tsx
"use client";
import React, { useEffect, useRef, useState } from "react";
// import { getSocket } from "@/utils/server"; // adjust path if needed
import { apiRequest } from '@/utils/ApiHelper';
import { getToken } from '@/utils/ApiCalls';
// import { io } from "socket.io-client";
type BetsMap = { [key: string]: number };

const ModernRoulette: React.FC = () => {
...
useEffect(() => {
  ...
  // Keep track of last wheelRotation value in a ref to compute deltas
  ...
  const placeBet = (position: string) => {
    ...
    setBets((prev) => {
      const copy = { ...prev };
      copy[position] = (copy[position] || 0) + betAmount;
      return copy;
    });
  ...
  const spin = () => {
    ...
    // Start spinning animation locally (visual)
    // give it some dramatic spins while waiting for server
    const startingSpins = 8 + Math.random() * 4;
    const startingRotation = startingSpins * 360;
    setWheelRotation((prev) => prev + startingRotation);
    setBallRotation((prev) => prev - (15 + Math.random() * 10) * 360);
    setBallVisible(true);

    // send to server: bets object and additional metadata
    const payload = {
      bets,
      totalBet,
      // include user id / session if available on your server auth;
      // server can also infer user from token
    };

    // Emit and optionally receive ack (server should determine result and emit 'roulette-result' to room or client).
    socket.emit("place-bets", payload, (ack: any) => {
      // ack example: { ok: true } or { ok: false, error: "..." }
      if (ack && ack.ok === false) {
        // server rejected bet (e.g., insufficient funds server-side)
        alert(ack.error || "Bet rejected by server");
        // revert local bets so UX doesn't break
        setBalance((prev) => prev + totalBet);
        setBets({});
        setTotalBet(0);
        setIsSpinning(false);
      }
      // otherwise wait for 'roulette-result' event
    });

    // If server doesn't respond within X seconds, you can optionally cancel spin (not implemented here).
  };

  /* ---------- Handle result from server ---------- */
  const handleServerResult = (winningNumber: number) => {
    // align wheel smoothly so winning pocket lands under the pointer.
    // compute index and target angle like local logic does.
    const winningIndex = wheelNumbers.indexOf(winningNumber);
    const segmentAngle = 360 / wheelNumbers.length; // 360/37
    const winningSegmentAngle = winningIndex * segmentAngle;

    // Important: we want the wheel to spin extra rotations before landing on the target
    // Compute an extra spins count to make it feel natural AFTER server triggers
    const extraSpins = 3 + Math.random() * 3; // 3..6 extra rotations
    const extraRotation = extraSpins * 360;

    // targetAngle is negative so the winning segment appears at top (pointer)
    const targetAngle = -winningSegmentAngle;

    // Final rotation adds on top of current visual rotation
    const finalRotation = wheelRotationRef.current + extraRotation + targetAngle;

    // Apply final rotation (this will animate because CSS has transition)
    setWheelRotation(finalRotation);

    // Ball should continue to rotate and then settle into pocket opposite direction
    const ballExtra = 8 + Math.random() * 6;
    const finalBallRotation = ballRotation - ballExtra * 360 - winningSegmentAngle;
    setBallRotation(finalBallRotation);

    // show ball until near landing
    setBallVisible(true);
    // show winning alert shortly after the wheel is mostly done
    // We'll wait about 4.2s to hide ball, then reveal result at 4.5s — adjust if your CSS transition duration differs
    ...
    // Complete the result flow after animation completes (5.5s)
    setTimeout(() => {
      setGameHistory((prev) => [...prev, winningNumber]);
      calculateWinnings(winningNumber);
      setIsSpinning(false);
      setTimeout(() => setShowWinningAlert(false), 3000);
    }, 5500);
  };

  /* ---------- Payout calculation (same as your logic) ---------- */
  const calculateWinnings = (winningNumber: number) => {
    ...
    Object.entries(bets).forEach(([position, amount]) => {
      ...
      if (position === winningNumber.toString()) {
        totalWinnings += a * 36; // straight number 35:1 + original
  ...
  /* ---------- Utility render helpers (kept same as your original) ---------- */
  ...
  /* ---------- JSX (kept your UI mostly intact) ---------- */
  return (
      ...
      <div className="bg-gray-800 border-b border-gray-700 p-4">
        <div className="flex justify-between items-center max-w-7xl mx-auto">
          <div className="flex items-center gap-4">
            ...
            <div className="text-sm text-gray-400">Live Casino (Server-controlled)</div>
          </div>
