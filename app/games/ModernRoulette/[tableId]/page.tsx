...
useEffect(() => {
  if (!spinFinished || !resolutions.length) return;
  ...
  if (
    ...
    lastShownBetIdRef.current = newResolvedBetId;
    setShowResultStatus(true); // ✅ WIN / LOSE UI
  }
}, [resolutions, spinFinished]);
...
useEffect(() => {
  ...
  const timer = setTimeout(() => {
   ;
    ...
    lastShownBetIdRef.current = null;
  }, 5000);
...
const resetGame = () => {
  ...
  setBets({});        // ✅ CLEAR BETS HERE
  setTotalBet(0);
  setIsSpinning(false); // safety
};
