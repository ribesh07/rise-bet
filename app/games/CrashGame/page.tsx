...
//   type ActivePlayer = { name: string; bet: number; multiplier: number | null };
...
const CrashGame: React.FC = () => {
  ...
  const [gameState, setGameState] = useState<'waiting' | 'betting' | 'flying' | 'crashed'>('waiting');
  ...
  const startNewGame = useCallback(() => {
    ...
    setGameHash(Math.random().toString(36).substring(2, 11));
  ...
  const performCashOut = useCallback((multiplier: number) => {
    ...
    setCashedOut(true);
    setPlayerBet(prev => prev ? { ...prev, multiplier } : null);

    const now = new Date();
    const timeString = `${now.getHours().toString().padStart(2, '0')}:${now.getMinutes().toString().padStart(2, '0')}`;
    setChatMessages(prev => [
      { user: 'You', message: `Cashed out at ${multiplier}x`, time: timeString },
      ...prev.slice(0, 9),
    ]);
  }, [playerBet, cashedOut, gameState]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900 text-white">
      {/* Render your UI exactly like before */}
      {/* ... */}
      <h1 className="text-3xl font-bold text-yellow-400">🚀 CRASH</h1>
    </div>
  );
