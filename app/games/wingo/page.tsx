...
export default function WingoFull() {
  ...
  // ⭐ ADDED — prevent betting confirmation
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex-1">
            ...
            <div className="w-full h-44 rounded-xl flex items-center justify-center mb-4 relative overflow-hidden"
              ...
              {/* ⭐ ADDED — last 5 second huge countdown overlay */}
              {timeLeft <= 5 && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/60 z-30 rounded-xl">
                  <div className="flex gap-3">
                    <div className="bg-white text-amber-600 font-extrabold text-7xl rounded-2xl px-6 py-3 shadow-xl">
                      {timeLeft}
                    </div>
                  </div>
                </div>
            ...
            {/* COLOR BUTTONS — ⭐ bettingLocked added */}
            <div className="grid grid-cols-3 gap-2 mb-3">
              <button disabled={bettingLocked} onClick={()=>onSelectColor('green')}
                className={`py-2 rounded ${bettingLocked?'opacity-50':'bg-green-600 text-white'}`}>
                Green
              </button>
              <button disabled={bettingLocked} onClick={()=>onSelectColor('violet')}
                className={`py-2 rounded ${bettingLocked?'opacity-50':'bg-purple-600 text-white'}`}>
                Violet
              </button>
              <button disabled={bettingLocked} onClick={()=>onSelectColor('red')}
                className={`py-2 rounded ${bettingLocked?'opacity-50':'bg-red-600 text-white'}`}>
                Red
              </button>
            </div>
            ...
            <div className="grid grid-cols-5 gap-2 mb-3">
              {NUMBERS.map(n=> (
                <button key={n} disabled={bettingLocked} onClick={()=>onSelectNumber(n)}
                  className={`p-1 rounded-xl border-2 ${bettingLocked?'opacity-40':'border-transparent'}`}>
            ...
            <div className="flex gap-2 items-center mb-3">
              <button disabled={bettingLocked} onClick={()=>onSelectBigSmall('big')}
                className={`flex-1 py-2 rounded ${bettingLocked?'opacity-40':'bg-green-500 text-white'}`}>
                Big
              </button>
              <button disabled={bettingLocked} onClick={()=>onSelectBigSmall('small')}
                className={`flex-1 py-2 rounded ${bettingLocked?'opacity-40':'bg-blue-500 text-white'}`}>
                Small
              </button>
            </div>
          </div>
